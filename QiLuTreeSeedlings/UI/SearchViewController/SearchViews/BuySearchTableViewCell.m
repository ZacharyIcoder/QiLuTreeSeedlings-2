...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIImageView * timeImag=[[UIImageView alloc]initWithFrame:CGRectMake(kWidth*0.5-25,38, 15, 15)];
         [timeImag setImage:[UIImage imageNamed:@"listtime"]];
        [self addSubview:timeImag];
        self.timeLab=[[UILabel alloc]initWithFrame:CGRectMake(kWidth*0.5, 40, 30, 12)];
        [self.timeLab setFont:[UIFont systemFontOfSize:10]];
        [self.timeLab setText:@"N天前"];
        [self addSubview:self.timeLab];
        UILabel *priceLab=[[UILabel alloc]initWithFrame:CGRectMake(kWidth*0.8-15, 40, 30, 12)];
        [priceLab setFont:[UIFont systemFontOfSize:10]];
        [priceLab setText:@"价格"];
        [self addSubview:priceLab];
        self.priceLab=[[UILabel alloc]initWithFrame:CGRectMake(kWidth*0.9-15, 35, 45, 20)];
...
-(void)setHotBuyModel:(HotBuyModel *)hotBuyModel
{
    ...
    self.timeLab=[timeAry firstObject];
    self.priceLab.text=hotBuyModel.price;
}
- (void)awakeFromNib {
    // Initialization code
}
